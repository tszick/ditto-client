...
from .client_internal import namespace_headers, raise_if_atomic_http_unsupported
...
class DittoHttpClient(DittoHttpClientBase):
    ...
    def get(self, key: str, namespace: str | None = None) -> DittoGetResult | None:
        ...
        status, body = self._request(
            ...
            extra_headers=namespace_headers(namespace),
        )
        if status == 404:
            return None
    ...
    def set_nx(
        ...
        raise_if_atomic_http_unsupported(status, body, "SET_NX")
        self._assert_ok(status, body)
        data = json.loads(body)
    ...
    def incr(
        ...
        raise_if_atomic_http_unsupported(status, body, "INCR")
        self._assert_ok(status, body)
        data = json.loads(body)
